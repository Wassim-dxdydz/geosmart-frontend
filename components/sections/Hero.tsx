...
export default function Hero({ lang = "fr" }: { lang?: "fr" | "en" }) {
  ...
  const results = [
    { label: t.cohesion,    value: "18 kPa", sub: "+2.3%", color: "text-emerald-400" },
    { label: t.angle,       value: "32°",    sub: "±0.5°", color: "text-amber-400"   },
    { label: t.soil,        value: t.soilVal, sub: "",      color: "text-white"       },
    { label: t.confidence,  value: "97.4%",  sub: "↑",     color: "text-indigo-400"  },
  ];
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden pt-4">
      ...
      <div className="absolute top-1/2 right-1/4 w-64 h-64 bg-amber-500/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto px-6 w-full grid md:grid-cols-2 gap-12 items-center py-20">
        ...
        <div className="flex flex-col gap-6">
          ...
          <motion.p {...fadeUp(0.3)} className="text-xl md:text-2xl text-amber-400 font-light leading-snug">
            {t.subtitle}
          </motion.p>
          ...
          <motion.div {...fadeUp(0.5)} className="flex flex-wrap gap-3 pt-2">
            ...
            <Link
              href="#features"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-white/10 hover:border-white/30 text-gray-300 hover:text-white font-semibold text-sm transition-all duration-200 hover:bg-white/5"
            >
              {t.cta2}
            </Link>
          </motion.div>
        ...
        >
          <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-5 shadow-2xl">
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wider">{t.cardLabel}</p>
                <p className="text-sm font-semibold text-white mt-0.5">{t.cardTitle}</p>
              </div>
            ...
            {/* Plot — no wrapper bg, fully transparent */}
            <div className="my-8">
                <MohrCircleSVG />
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-3">
              {results.map((item) => (
                <div key={item.label} className="bg-white/5 rounded-lg px-3 py-2.5 border border-white/5">
...
function MohrCircleSVG() {
  // All circles share cy = oy (on the σ-axis)
  // Manually positioned for maximum visual spread across the full plot width
  // Small: indigo low opacity | Medium: indigo mid opacity | Large: amber full

  const oy = 210; // σ-axis y position

  const circles = [
    { cx: 170, r: 50,  stroke: "#6366F1", opacity: 0.45, sw: 1.5 }, // small
    { cx: 285, r: 80, stroke: "#6366F1", opacity: 0.75, sw: 1.8 }, // medium
    { cx: 440, r: 120, stroke: "#F59E0B", opacity: 1.00, sw: 2.2 }, // large
  ];
  ...
  return (
    ...
    >
      ...
      <text x="576" y={oy + 16} fill="#334155" fontSize="8">(kPa)</text>
      ...
      <text x="10" y="28" fill="#334155" fontSize="8">(kPa)</text>

      {/* σ axis tick labels — every 80px = 100 kPa visually */}
      {["100", "200", "300", "400", "500"].map((label, i) => {
        const x = 80 + (i + 1) * gridStep;
        return (
          <g key={label}>
            <line x1={x} y1={oy} x2={x} y2={oy + 5} stroke="#334155" strokeWidth="0.8" />
            <text x={x - 8} y={oy + 14} fill="#334155" fontSize="8">{label}</text>
          </g>
        );
      })}
      ...
      {["50", "100", "150"].map((label, i) => {
        ...
        return (
          <g key={label}>
            <line x1="64" y1={y} x2="72" y2={y} stroke="#334155" strokeWidth="0.8" />
            <text x="20" y={y + 3} fill="#334155" fontSize="8">{label}</text>
          </g>
        );
      })}

      {/* ── Failure envelope ── */}
      {/* Starts at c-intercept on τ-axis, tangent to all circles */}
      <line
        x1="68"  y1="185"
        x2="500" y2="68"
        stroke="#6366F1"
      ...
      {/* ── φ arc at origin ── */}
      <path
        d="M 87 210 A 30 30 0 0 0 82 183"
        fill="none"
        stroke="#F59E0B"
        strokeWidth="2"
        opacity="0.6"
